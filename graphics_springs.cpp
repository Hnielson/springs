...
void display(void)
{
	...
	for(i=0; i<NF; i++)
	{
		...
		if(f->Type() == SPRING_FORCE)
		{
			...
			glColor3ub(sf->GetRed(), sf->GetGreen(), sf->GetBlue());
...
void keyboard(unsigned char c, int x, int y)
{
	switch (c) 
	{
		case 27: // escape character means to quit the program
			...
			break;
		case '1':
			PS.SetDeltaT(PS.GetDeltaT());
			break;
		case '2':
			PS.SetDeltaT(PS.GetDeltaT() * 2);
			break;
		case '3':
			PS.SetDeltaT(PS.GetDeltaT() * .5);
			break;
		default:
...
void ReadfromFile()
{
	...
	while (!fin.eof())
	{
		...
		if (strcmp(type, "particle") == 0)
		{
			...
			Particle* p = new Particle(x, y, dx, dy, rad, fixed);
			PS.AddParticle(p);
		}
		else if (strcmp(type, "spring") == 0)
		{
			...
			Force* f = new SpringForce(PS.GetParticle(p1), PS.GetParticle(p2), springiness, damping, r, g, b, restlength=0);
			PS.AddForce(f);
		}
		...
		else if (strcmp(type, "gravity") == 0)
		{
			...
			Force* g = new GravityForce(gravity, &PS);
			PS.AddForce(g);
		}
		else if (strcmp(type, "drag") == 0)
		{
			double drag;
			fin >> drag >> ws;
			Force* d = new DragForce(drag, &PS);
			PS.AddForce(d);
		}
		else
			cout << "No legal object" << endl;
	}
}
